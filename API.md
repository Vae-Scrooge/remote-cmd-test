...
class SSHClient:
    ...
    def execute(self, command: str, timeout: Optional[int] = None, environment: Optional[Dict[str, str]] = None) -> CommandResult
    def execute_sudo(self, command: str, password: Optional[str] = None, timeout: Optional[int] = None) -> CommandResult
...
**参数：**
- `command` - 要执行的命令
- `password` - sudo 密码（如果配置了免密 sudo 可不传）
- `timeout` - 超时时间
...
# 批量添加主机
hosts_data = [
    {"name": "web-01", "hostname": "192.168.1.10", "username": "ubuntu", "tags": ["web"]},
    {"name": "web-02", "hostname": "192.168.1.11", "username": "ubuntu", "tags": ["web"]},
    {"name": "db-01", "hostname": "192.168.1.20", "username": "admin", "tags": ["database"]},
]

for data in hosts_data:
    host = Host(**data, key_filename="~/.ssh/id_rsa")
    manager.add_host(host)
...
for host in manager.list_hosts(tag="web"):
    with manager.connect_to_host(host.name) as client:
        ...
        print(f"{status} {host.name}: Nginx")
```
...
def safe_execute(config: ConnectionConfig, command: str):
    """安全执行命令，带完整错误处理"""
    try:
        with SSHClient(config) as client:
...
# 使用
result = safe_execute(config, "ls -la")
if result["success"]:
    print(result["output"])
else:
